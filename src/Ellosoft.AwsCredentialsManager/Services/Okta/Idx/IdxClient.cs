...
public sealed class IdxClient(HttpClient httpClient) : IDisposable
{
    ...
    /// <summary>
    ///     GET used for <c>redirect-idp</c> remediations (Windows FastPass). The response may be Ion JSON,
    ///     an HTML page that embeds the Okta Verify deep link, or a redirect to that deep link.
    /// </summary>
...
public sealed record IdxRawResponse(int StatusCode, string Content, string? Location, string? MediaType);
