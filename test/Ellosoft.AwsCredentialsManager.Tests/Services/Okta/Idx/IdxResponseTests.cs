...
public class IdxResponseTests
{
    ...
    [Fact]
    public void RedirectIdp_ShouldExposeSameOriginFastPassHref()
    {
        var response = IdxResponse.Parse(IdxPayloads.RedirectIdpFastPass);

        response.RemediationNames.ShouldBe(["redirect-idp"]);
        var redirect = response.GetRemediation(IdxResponse.RedirectIdpRemediation).ShouldNotBeNull();
        redirect.Href.ShouldBe("https://xyz.okta.com/sso/idps/0oa-fastpass");
        response.PollRemediation.ShouldBeNull();
        response.GetRemediation(IdxResponse.LaunchAuthenticatorRemediation).ShouldBeNull();
    }
